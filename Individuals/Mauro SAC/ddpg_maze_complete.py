...
Run:
    python ddpg_maze_complete.py              # full training + test
    python ddpg_maze_complete.py --test-only  # reproduce test without retraining
...
Observation (7 features):
  [norm_row, norm_col, norm_bfs_dist, can_up, can_down, can_left, can_right]
...
Reward system:
  ...
  -3.0       wall collision
  +0.5       first visit to a new cell (exploration bonus)
  -1.0*n     revisiting cell n times (capped at -20)
  +(6-d)     proximity bonus when BFS distance d <= 5
...
class MazeEnv:
    """
    16x16 grid maze compatible with the Gymnasium API.

    Agent starts at (0, 0) — top-left.
    Agent must reach (15, 15) — bottom-right.
    ...
    def __init__(self):
        ...
        # Wall layout — set of (row, col) tuples.
        # Same maze as maze_1/maze_1.py — coordinates converted from
        # maze_1's (x=col, y=row) format to our (row, col) format.
        self.walls = {
            # vertical col=3, rows 1-5
            (1,3),(2,3),(3,3),(4,3),(5,3),
            # vertical col=6, rows 2-6
            (2,6),(3,6),(4,6),(5,6),(6,6),
            # vertical col=9, rows 1-4
            (1,9),(2,9),(3,9),(4,9),
            # vertical col=12, rows 3-7
            (3,12),(4,12),(5,12),(6,12),(7,12),
            # vertical col=2, rows 8-12
            (8,2),(9,2),(10,2),(11,2),(12,2),
            # vertical col=5, rows 9-14
            (9,5),(10,5),(11,5),(12,5),(13,5),(14,5),
            # vertical col=8, rows 7-11
            (7,8),(8,8),(9,8),(10,8),(11,8),
            # vertical col=11, rows 10-14
            (10,11),(11,11),(12,11),(13,11),(14,11),
            # horizontal row=4, cols 1-4
            (4,1),(4,2),(4,3),(4,4),
            # horizontal row=8, cols 6-10
            (8,6),(8,7),(8,8),(8,9),(8,10),
            # horizontal row=12, cols 10-14
            (12,10),(12,11),(12,12),(12,13),(12,14),
            # horizontal row=1, cols 12-14
            (1,12),(1,13),(1,14),
            # horizontal row=10, cols 0-1
            (10,0),(10,1),
            # horizontal row=5, cols 13-15
            (5,13),(5,14),(5,15),
            # extra walls
            (6,1),(8,4),(13,7),(6,10),(9,14),
        }

        self.start_pos = (0, 0)
        self.exit_pos  = (self.GRID_ROWS - 1, self.GRID_COLS - 1)  # (15,15)
        ...
        # BFS distance map: precomputed once at init.
        # bfs_dist[(r,c)] = shortest path steps from (r,c) to exit through the
        # actual maze (respects walls). Replaces Manhattan distance for an
        # accurate signal even when walls block the straight-line path.
        self.bfs_dist     = self._compute_bfs_distances()
        self.max_bfs_dist = max(self.bfs_dist.values()) if self.bfs_dist else 30

        self.action_space = spaces.Discrete(4)

        # 7-float observation: position (2) + BFS distance (1) + wall flags (4)
    ...
    def _distance_to_exit(self):
        """BFS shortest-path distance — accurate even when walls block the straight line."""
    ...
    def step(self, action):
        ...
        # 1. Step penalty — constant cost encourages efficiency
        reward -= 0.1
        reward_breakdown.append(('step_penalty', -0.1))
        ...
        if moved:
            ...
            if n == 0:
                reward += 0.5
                reward_breakdown.append(('exploration_bonus', +0.5))
            else:
                penalty = min(1.0 * n, 20.0)
                reward -= penalty
                reward_breakdown.append(('revisit_penalty', -round(penalty, 2)))
            self.visit_count[new_pos] = n + 1

        # 5. Proximity bonus — extra incentive within 5 BFS steps of exit.
        # Guides the agent over the last mile where Q-values are sparse.
        if moved and new_dist <= 5:
            prox = float(6 - new_dist)   # dist=1 -> +5, dist=5 -> +1
            reward += prox
...
def draw_maze(env, path=None, agent_pos=None, title='16x16 Maze',
    ...
    """Draw the 16x16 maze. Saves to PNG when called standalone (no ax provided)."""
    ...
    if standalone:
        plt.tight_layout()
        plt.savefig('maze_layout.png', dpi=120, bbox_inches='tight')
...
class ContinuousMazeWrapper(gym.Env):
    ...
    The discrete action is: argmax of those 4 scores.
...
class TrainingRecorderCallback(BaseCallback):
    """
    Runs a greedy evaluation episode at the end of every N training episodes
    and stores the path, reward, and distance to exit.

    Saves step snapshots for the training GIF and checkpoints the best model
    by reward AND by minimum distance reached (dual criterion).
    """
...
def train(timesteps=30_000, save_path='models/sac_maze_v3', record_every=1):
    """
    ...
    - More sample-efficient: converges in ~20k timesteps on this maze
    ...
    # snap_every=60 -> 30000/60 = 500 snapshots -> ~33s GIF at 15fps
    callback = TrainingRecorderCallback(
        record_every=record_every, snap_every=60,
...
def run_episode_logged(model, env_wrapper, max_steps=500, deterministic=True):
...
def save_episode_animation(env_wrapper, model, filename='episode_animation.gif',
                           max_steps=500, fps=6):
    """Run one episode and save it as an animated GIF."""
...
def save_all_steps_gif(env, step_snapshots, filename='training_all_steps.gif', fps=15):
    """Animate every recorded training step snapshot."""
...
def main():
    ...
    if args.test_only:
        ...
        model, training_history, step_snapshots, best_path = train(
            timesteps=30_000, save_path=model_path, record_every=1
        )
        used_model = best_path if os.path.exists(best_path + '.zip') else model_path
        print()

    # Maze layout with BFS optimal path
    print('Computing shortest path (BFS)...')
    env_layout = MazeEnv()
    env_layout.reset()
    bfs_path = find_shortest_path(env_layout)
    if bfs_path:
        print(f'  Shortest path: {len(bfs_path)} steps')
        draw_maze(env_layout, path=bfs_path,
                  title=f'Maze layout -- optimal path ({len(bfs_path)} steps, BFS)')
    else:
        print('  WARNING: no path found!')
        draw_maze(env_layout, title='Maze layout')
    ...
    log, path, success, total_reward = run_episode_logged(
        model_ep, env_ep, max_steps=500, deterministic=True
    )
